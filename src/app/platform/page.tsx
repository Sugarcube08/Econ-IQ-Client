      ...
      mock: (
        <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-4 shadow-sm font-sans">
          <div className="flex justify-between items-center pb-2 border-b border-outline-variant/60">
            ...
            <span className="text-[10px] text-brand-accent font-bold bg-brand-accent/10 px-2 py-0.5 rounded-full uppercase">Live Sync</span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-background p-3 rounded border border-outline-variant/40">
              <span className="text-[10px] text-outline uppercase block">DSO (Current)</span>
              <span className="text-lg font-black text-secondary">38.2 Days</span>
              <span className="text-[9px] text-brand-accent block mt-0.5 font-bold">-4.5d vs Last Month</span>
            </div>
            <div className="bg-background p-3 rounded border border-outline-variant/40">
          ...
          <div className="space-y-1">
            <span className="text-[9px] text-outline uppercase font-bold block">Cash Inflow Trend (12 Months)</span>
            <div className="h-16 flex items-end gap-1 pt-2">
              {[30, 45, 35, 60, 50, 75, 55, 80, 70, 95, 85, 100].map((h, i) => (
                <div key={i} className="flex-1 bg-brand-accent/20 rounded-t" style={{ height: `${h}%` }}></div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-4 shadow-sm font-sans">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-secondary">Vohra-Dugal FMCG</span>
              <span className="font-mono text-outline">ID: 381f54d5</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-background p-2 rounded text-center border border-outline-variant/40">
                <span className="text-[8px] text-outline uppercase block">Trust Score</span>
                <span className="text-xs font-bold text-secondary">52%</span>
              </div>
              <div className="bg-background p-2 rounded text-center border border-outline-variant/40">
                <span className="text-[8px] text-outline uppercase block">Payment Score</span>
                <span className="text-xs font-bold text-secondary">41%</span>
              </div>
              <div className="bg-background p-2 rounded text-center border border-outline-variant/40">
                <span className="text-[8px] text-outline uppercase block">Growth Score</span>
      ...
      mock: (
        <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-4 shadow-sm font-sans">
          <div className="flex justify-between items-center pb-2 border-b border-outline-variant/60">
            ...
            <span className="text-[10px] text-brand-accent font-bold bg-brand-accent/10 px-2 py-0.5 rounded-full uppercase">Calculated</span>
          </div>
          <div className="space-y-3">
            <div className="text-[10px] text-outline uppercase font-bold">Policy Recommendation</div>
            <div className="p-3 bg-background rounded border border-outline-variant space-y-1">
              <div className="text-xs font-bold text-secondary uppercase">Adjust Payment Terms</div>
      ...
      mock: (
        <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-4 shadow-sm font-sans">
          <div className="flex justify-between items-center pb-2 border-b border-outline-variant/60">
            ...
            <span className="material-symbols-outlined text-brand-accent text-[18px]">download</span>
          </div>
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-[9px] text-outline uppercase block font-bold">Filter Dataset</label>
              <div className="text-xs bg-background border border-outline-variant p-2 rounded text-secondary font-mono">
                SELECT * FROM ledger WHERE risk_state = &apos;liquidity_stress&apos;
              </div>
            </div>
            <div className="flex gap-2">
              <button className="flex-1 py-1.5 bg-brand-accent text-white text-[10px] font-bold uppercase rounded text-center">
                Export CSV
              </button>
              <button className="flex-1 py-1.5 border border-outline-variant text-secondary text-[10px] font-bold uppercase rounded text-center">
                Export PDF
              </button>
            </div>
      ...
      mock: (
        <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-4 shadow-sm font-sans">
          <div className="flex justify-between items-center pb-2 border-b border-outline-variant/60">
            ...
            <span className="text-[10px] text-brand-accent font-bold bg-brand-accent/10 px-2 py-0.5 rounded-full uppercase">Compliant</span>
          </div>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between items-center p-2 bg-background rounded border border-outline-variant/40">
              <span>Data Encryption (AES-256)</span>
              <span className="material-symbols-outlined text-brand-accent text-[16px]">verified</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-background rounded border border-outline-variant/40">
    ...
    <div className="space-y-24 py-16 px-6 lg:px-8 bg-background text-secondary">
      ...
      <section className="max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-5 space-y-4">
          <span className="text-[10px] font-bold text-outline uppercase tracking-wider block mb-2">Platform Modules</span>
                ...
                <button
                  ...
                  className={`w-full p-4 rounded-lg text-left border transition-all duration-300 flex items-center justify-between cursor-pointer ${
                    isActive
                      ...
                      : 'bg-background border-transparent hover:bg-white/40 hover:border-outline-variant'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${
                      isActive ? 'bg-brand-accent text-white' : 'bg-brand-accent/10 text-brand-accent'
                    }`}>
                      <span className="material-symbols-outlined text-[18px]">{m.icon}</span>
                    </div>
                    <div>
                      <h4 className={`text-xs font-bold uppercase tracking-wider ${isActive ? 'text-secondary' : 'text-outline'}`}>
                        {m.name}
                      </h4>
                      <p className="text-[10px] text-outline mt-0.5">{m.tagline}</p>
                    </div>
                  </div>
                  <span className={`material-symbols-outlined text-[16px] text-outline transition-transform duration-300 ${
                    isActive ? 'translate-x-1 text-brand-accent' : ''
                  }`}>
                    chevron_right
                  </span>
                </button>
        ...
        <div className="lg:col-span-7 space-y-8 bg-white border border-outline-variant rounded-xl p-8 shadow-sm min-h-[440px] flex flex-col justify-between">
          {modules.map((m) => {
            ...
            return (
              <div key={m.id} className="space-y-8 animate-fade-in flex-grow flex flex-col justify-between">
                <div className="space-y-6">
                  <div>
                    <h3 className="font-headline text-xl font-extrabold text-secondary">{m.name}</h3>
                    <p className="font-sans text-xs text-outline mt-1">{m.tagline}</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs border-y border-outline-variant/60 py-6">
                    <div className="space-y-1">
                      <strong className="text-[10px] text-[#ba1a1a] uppercase font-bold tracking-wider">The Problem</strong>
                      <p className="font-sans text-outline leading-relaxed">{m.problem}</p>
                    </div>
                    <div className="space-y-1">
                      <strong className="text-[10px] text-brand-accent uppercase font-bold tracking-wider">The Solution</strong>
                      <p className="font-sans text-outline leading-relaxed">{m.solution}</p>
                    </div>
                    <div className="space-y-1">
                      <strong className="text-[10px] text-secondary uppercase font-bold tracking-wider">The Outcome</strong>
                      <p className="font-sans text-secondary font-semibold leading-relaxed">{m.outcome}</p>
                    </div>
                  </div>
                </div>
      ...
      <section className="bg-white border border-outline-variant rounded-xl p-8 md:p-12 max-w-[1280px] mx-auto space-y-8">
        <div className="text-center space-y-2 max-w-2xl mx-auto">
          <h3 className="font-headline text-lg font-bold text-secondary">Active Ledger Verification Flow</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-xs text-center">
          <div className="p-6 bg-background rounded border border-outline-variant/60 space-y-2">
            <div className="w-8 h-8 rounded-full bg-brand-accent text-white flex items-center justify-center font-bold mx-auto font-mono">1</div>
            <strong className="font-headline text-secondary uppercase block tracking-wider text-[10px]">Ingestion</strong>
            <p className="font-sans text-outline leading-relaxed">Secure REST APIs or ledger file CSV transfers load invoices.</p>
          </div>
          <div className="p-6 bg-background rounded border border-outline-variant/60 space-y-2">
            <div className="w-8 h-8 rounded-full bg-brand-accent text-white flex items-center justify-center font-bold mx-auto font-mono">2</div>
            <strong className="font-headline text-secondary uppercase block tracking-wider text-[10px]">Normalization</strong>
            <p className="font-sans text-outline leading-relaxed">Dates, balances, and customer details are unified and clean.</p>
          </div>
          <div className="p-6 bg-background rounded border border-outline-variant/60 space-y-2">
            <div className="w-8 h-8 rounded-full bg-brand-accent text-white flex items-center justify-center font-bold mx-auto font-mono">3</div>
            <strong className="font-headline text-secondary uppercase block tracking-wider text-[10px]">Scoring</strong>
            <p className="font-sans text-outline leading-relaxed">Statistical algorithms update continuous risk and trust profiles.</p>
          </div>
          <div className="p-6 bg-background rounded border border-outline-variant/60 space-y-2">
            <div className="w-8 h-8 rounded-full bg-brand-accent text-white flex items-center justify-center font-bold mx-auto font-mono">4</div>
            <strong className="font-headline text-secondary uppercase block tracking-wider text-[10px]">Action</strong>
            <p className="font-sans text-outline leading-relaxed">Predictive alerts recommend payment terms or credit adjustments.</p>
          </div>
        </div>
      </section>
