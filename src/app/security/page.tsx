    ...
    <div className="space-y-24 py-16 px-6 lg:px-8 bg-background text-secondary">
      ...
      <section className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 font-sans text-xs">
        {securityControls.map((sc, i) => (
          <div key={i} className="bg-white p-8 rounded-xl border border-outline-variant space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="w-10 h-10 rounded bg-brand-accent/10 flex items-center justify-center text-brand-accent">
              <span className="material-symbols-outlined text-[20px]">{sc.icon}</span>
            </div>
            <h3 className="font-headline text-sm font-bold text-secondary uppercase tracking-wider">{sc.title}</h3>
            <p className="text-outline leading-relaxed">{sc.description}</p>
          </div>
        ))}
      </section>

      {/* Compliance statement */}
      <section className="bg-white border border-outline-variant rounded-xl p-8 md:p-12 max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center font-sans text-xs">
        <div className="space-y-4">
          <span className="text-[10px] font-bold text-brand-accent uppercase tracking-wider block">Compliance Standards</span>
          <h3 className="font-headline text-lg font-bold text-secondary">SOC-2 & ISO 27001 Alignment</h3>
        ...
        <div className="p-6 bg-background rounded border border-outline-variant/60 space-y-2 text-[10px] text-outline font-semibold">
          <div className="flex justify-between items-center py-1 border-b border-outline-variant/40">
            ...
            <span className="text-brand-accent font-bold">Compliant / Active</span>
          </div>
          <div className="flex justify-between items-center py-1 border-b border-outline-variant/40">
            <span>Encryption Standard:</span>
            <span className="text-brand-accent font-bold">AES-256-GCM</span>
          </div>
          <div className="flex justify-between items-center py-1">
            <span>Database Ingestion:</span>
            <span className="text-brand-accent font-bold">Read-Only Connectors</span>
          </div>
        </div>
      </section>
